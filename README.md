...
OTE, OTL, OTU, CLR, MOV, NEG, ADD, SUB, MUL, DIV, TON, RTO, TOF, RES, CTU,
and CTD only when a record
...
CLR reports `destination`. MOV and NEG report `source` and `destination`. ADD,
SUB, MUL, and DIV report `source_a`, `source_b`, and `destination`. Meanwhile,
