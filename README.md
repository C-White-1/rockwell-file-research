...
DIV, DCD, ENC, EQU, NEQ, GRT, GEQ, LES, LEQ, MEQ, LIM, SCP, SCL, SWP, COP, FLL,
...
LIM reports `low_limit`, `test`, and `high_limit`.
SCP reports `input`, `input_min`, `input_max`, `scaled_min`, `scaled_max`, and
`output`.
SCL reports `source`, `rate`, `offset`, and `destination`.
...
TOD, FRD, DCD, and ENC report `source` and `destination`.
Meanwhile,
...
RET reports no operands and acts as a subroutine-return marker.
MCR reports no operands; zone-pairing semantics are not inferred from an
isolated instruction record.
...
operands.
SQO reports ordered `file`, `mask`, `destination`, `control`, `length`, and
`position` operands.
