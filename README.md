...
OTE, OTL, OTU, CLR, MOV, NEG, SQR, ABS, AND, OR, ADD, SUB, MUL, DIV, TON,
...
CLR reports `destination`. MOV, NEG, SQR, and ABS report `source` and
`destination`. AND, OR, ADD, SUB, MUL, and DIV report `source_a`, `source_b`,
and `destination`.
Meanwhile,
