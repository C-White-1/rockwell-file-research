...
from rockwell_file_research.rss.instruction_evidence import (
    ...
    scan_controlled_rta_instructions,
...
def test_aic_exposes_integer_source_and_string_destination() -> None:
    result = scan_controlled_aic_instructions(
        _aic_record(),
        include_private_text=True,
    )

    assert len(result) == 1
    assert (result[0].mnemonic, result[0].selector) == ("AIC", 0x7F)
    assert [(item.role, item.value) for item in result[0].operands] == [
        ("source", "N7:0"),
        ("destination", "ST9:0"),
    ]
...
def test_rta_is_a_zero_operand_instruction() -> None:
    result = scan_controlled_rta_instructions(_zero_operand_record(0xAD))

    assert len(result) == 1
    assert (result[0].mnemonic, result[0].selector) == ("RTA", 0xAD)
    assert result[0].operands == ()
