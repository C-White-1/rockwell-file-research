...
_QUALIFIED_WORD_IDENTITIES = {
    ...
    0x64: ("ENC", _ENC_PROFILE, ("source", "destination"), 0x04),
    0xAF: ("GCD", _GCD_PROFILE, ("source", "destination"), 0x04),
    0x23: (
        "AND",
        _AND_PROFILE,
...
def scan_controlled_ahl_instructions(
    payload: bytes,
    *,
    include_private_text: bool = False,
) -> list[InstructionEvidence]:
    """Recognize AHL records matching the controlled handshake-lines profile."""
    ...
    for record_offset in range(max(0, len(payload) - 1)):
        ...
        if len(fields) != 6 or cursor + 10 > len(payload):
            continue
        if payload[cursor : cursor + 3] != b"\x00\x00\x7e":
            continue
        selector_offset = cursor + 2
        try:
            decoded = [value.decode("ascii") for _, value in fields]
...
def scan_controlled_asc_instructions(
    payload: bytes,
    *,
    include_private_text: bool = False,
) -> list[InstructionEvidence]:
    """Recognize ASC records matching the controlled ASCII-search profile."""
    ...
    for record_offset in range(max(0, len(payload) - 1)):
        ...
        for _ in range(4):
            ...
            fields.append((offset, payload[offset:end]))
            cursor = end
        if len(fields) != 4 or cursor + 12 > len(payload):
            continue
        if payload[cursor : cursor + 5] != b"\x01\x3f\x00\x00\x82":
            continue
        selector_offset = cursor + 4
        try:
            decoded = [value.decode("ascii") for _, value in fields]
        ...
        if payload[selector_offset + 1 : selector_offset + 8] != (
            ...
            continue
        roles = ("source", "index", "string_search", "result")
        evidence.append(
            InstructionEvidence(
                mnemonic="ASC",
                selector=0x82,
...
def scan_controlled_enc_instructions(
    payload: bytes,
    *,
    include_private_text: bool = False,
) -> list[InstructionEvidence]:
    """Recognize ENC records matching the controlled qualified-word profile."""
    ...
    return [
        ...
        if item.mnemonic == "ENC"
    ]


def scan_controlled_gcd_instructions(
    payload: bytes,
    *,
    include_private_text: bool = False,
) -> list[InstructionEvidence]:
    """Recognize GCD records matching the controlled qualified-word profile."""
