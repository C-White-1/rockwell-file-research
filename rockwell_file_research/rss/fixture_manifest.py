...
def validate_fixture_manifest(manifest: Path) -> list[ManifestIssue]:
    ...
    required_values = tuple(
        ...
        if column not in {"parent_fixture", "notes"}
    )
    ...
    for number, row in rows:
        ...
        if fixture_id in ids:
            issues.append(
                ManifestIssue(
                    f"duplicate fixture_id {fixture_id!r} (first seen on row {ids[fixture_id]})",
                    number,
                )
            )
        elif fixture_id:
        ...
        if filename in filenames:
            issues.append(
                ManifestIssue(
                    f"duplicate filename {filename!r} (first seen on row {filenames[filename]})",
                    number,
                )
            )
        elif filename:
        ...
        if source is None:
            issues.append(
                ManifestIssue("filename must be a safe relative .rss path", number)
            )
        else:
            if fixture_id and source.stem != fixture_id:
                issues.append(
                    ManifestIssue("fixture_id must equal the RSS filename stem", number)
                )
            if not source.is_file():
                issues.append(
                    ManifestIssue(f"RSS file does not exist: {filename}", number)
                )
            else:
                expected = (row.get("sha256") or "").strip()
                if _SHA256.fullmatch(expected) and _digest(source) != expected:
                    issues.append(
                        ManifestIssue("sha256 does not match the RSS file", number)
                    )
        ...
        if sha256 and not _SHA256.fullmatch(sha256):
            issues.append(
                ManifestIssue(
                    "sha256 must be 64 lowercase hexadecimal characters", number
                )
            )
        ...
        if timestamp and not _valid_timestamp(timestamp):
            issues.append(
                ManifestIssue(
                    "created_at must be an ISO 8601 timestamp with a UTC offset", number
                )
            )
    ...
    if len(roots) != 1:
        issues.append(
            ManifestIssue(
                f"manifest must contain exactly one root fixture; found {len(roots)}"
            )
        )
    for fixture_id, parent in parents.items():
        if parent and parent not in ids:
            issues.append(
                ManifestIssue(
                    f"fixture {fixture_id!r} references unknown parent {parent!r}",
                    ids[fixture_id],
                )
            )
    for fixture_id in sorted(_cycle_members(parents)):
        issues.append(
            ManifestIssue(
                f"fixture {fixture_id!r} participates in a parent cycle",
                ids[fixture_id],
            )
        )
    return issues
